...
test.describe('Jupiter Lazyload Module', () => {
  ...
  test('should initialize and mark images with appropriate attributes', async ({ page }) => {
    // First image should be initialized - specify more precisely
    const firstImage = page.locator('figure[data-orientation="landscape"]:first-of-type img')
    await expect(firstImage).toBeVisible()
    
    // Check for initialized attributes
    await expect(firstImage).toHaveAttribute('data-ll-blurred')
    
    // Check if parent picture element has the initialized attribute
    const firstPicture = page.locator('figure[data-orientation="landscape"]:first-of-type picture')
  ...
  test('should lazy load images when scrolled into view', async ({ page }) => {
    // Second image (below the fold) - specify more precisely
    const secondImage = page.locator('figure[data-orientation="portrait"] img')
    ...
    await expect(secondImage).toHaveAttribute('data-ll-ready', { timeout: 10000 })
    await expect(secondImage).toHaveAttribute('data-ll-loaded', { timeout: 10000 })
  ...
  test('should load all images in a lazyload section when section becomes visible', async ({ page }) => {
    // Get the lazyload section
    const lazyloadSection = page.locator('[data-lazyload-section]')
    const sectionImage = lazyloadSection.locator('img').first()
    ...
    await expect(sectionImage).toHaveAttribute('data-ll-ready', { timeout: 10000 })
    
    // Check if parent picture has the ready attribute
    const sectionPicture = lazyloadSection.locator('picture').first()
    await expect(sectionPicture).toHaveAttribute('data-ll-srcset-ready', { timeout: 10000 })
  })

  test('should update sizes attribute for auto-sized images', async ({ page }) => {
    // First image has data-sizes="auto" - specify more precisely
    const firstImage = page.locator('figure[data-orientation="landscape"]:first-of-type img')
